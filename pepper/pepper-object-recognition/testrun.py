...
class SetGetTest(ServiceTester):
    script = "app/scripts/pepperobjectrecognition.py"
    def run(self):
        time.sleep(1)

        PepperObjectRecognition = self.service("PepperObjectRecognition")
        PepperObjectRecognition.set(0)
        assert PepperObjectRecognition.get() == 0
        PepperObjectRecognition.set(2)
        assert PepperObjectRecognition.get() == 2
        return True
...
class StopTest(ServiceTester):
    ...
    def run(self):
        ...
        goterror = False
        try:
            PepperObjectRecognition.stop()
            time.sleep(1)
            PepperObjectRecognition.set(1)
        except RuntimeError:
